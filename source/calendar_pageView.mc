...
class CalendarPageView extends WatchUi.View {
    ...
    // Build YYYY-MM-DD string for the selected date
    function getSelectedDateKey() as String {
...
class CalendarPageDelegate extends WatchUi.InputDelegate {
    ...
    function onTap(clickEvent as ClickEvent) as Boolean {
        ...
        // Check if a date cell was tapped
        ...
        if (tappedDay > 0) {
            ...
            var dateKey = calendarView.getSelectedDateKey();
            var dateString = calendarView.getSelectedDateString();
            var hasData = calendarView.dateHasData(tappedDay);
            
            if (hasData) {
                // HAS DATA: Open the weekly chart centered on this date's week
                var chartView = new ChartView(dateKey);
                var chartDelegate = new ChartDelegate();
                chartDelegate.setView(chartView);
                WatchUi.pushView(chartView, chartDelegate, WatchUi.SLIDE_LEFT);
                return true;
            }
    ...
    function onKey(keyEvent as KeyEvent) as Boolean {
        ...
        if (key == WatchUi.KEY_ENTER) {
            ...
            if (hasData) {
                // Open weekly chart for that date's week
                var chartView = new ChartView(dateKey);
                var chartDelegate = new ChartDelegate();
                chartDelegate.setView(chartView);
                WatchUi.pushView(chartView, chartDelegate, WatchUi.SLIDE_LEFT);
